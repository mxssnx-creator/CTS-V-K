...
async function triggerStrategyProcessing() {
  try {
    ...
    const activeConnections = connections.filter(c => c.dashboard_enabled === 1);
    ...
    for (const conn of activeConnections) {
      ...
      const symbolsData = await client.hgetall(symbolsKey).catch(() => ({}));
      ...
      for (const symbol of symbols) {
        ...
        const marketData = await client.hgetall(marketDataKey).catch(() => ({}));
      ...
      for (const symbol of symbols) {
        ...
        try {
          ...
          // Run strategy flow
          const results = await processor.executeStrategyFlow(symbol, []);
          console.log(`✓ Strategy processing completed for ${symbol}:`, results.map(r => `${r.type}: ${r.totalCreated} created, ${r.passedEvaluation} passed`));
